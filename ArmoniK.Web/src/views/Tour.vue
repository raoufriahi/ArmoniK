      ...
      <!-- 左侧数据栏 -->
                            ...
                            <div style="width:100%;display: flex;justify-content: space-between;">
                                ...
                                <div style="display:flex">
                                    ...
                                    <span :class="data.iconclass3" class="nowplay" >
                                        <div style="display:flex">
                                            <span class="dot">●</span>
                                            <span class="nowplayText">Now Playing</span>
                                        </div>
                                    </span>
                                </div>
                            </div>
		...
		<!-- Video bar on the right -->
      	<div class="liveview_right" id="videoPanel">
            <div id="video_hed">
                <div name='flex' style="position: relative;" class="videoColor videogh"  v-for="r in rows" :key="r">
                    <div 
                        class="palace" 
                        name="flex" 
                        v-for="c in cols" 
            ...
            <div class="liveview_Tips">Select patrol mode</div>
		</div>
    </div>
</template>
...
import {listdatagload,listdatag,listdataChilde,AllMethods} from './public/regional'
export default {
    name: 'Liveview',
    components: {
        'True-play': Trueplay
	...
	methods:{
        ...
        Playall(){
            var _this=this
            _this.$store.state.stopTurePlay=true;
        ...
        Playview(){
            ...
            for(var i=0;i<data[0].src.length;i++){
                ...
                this.Tranvalue(item,i,data[0].src[i].strIndex)
            }
            data.push(data[0])
            data.shift()
            ...
            this.timersetInterval=setInterval(function(){
                ...
                for(var i=0;i<data[0].src.length;i++){
                    var item = data[0].src[i].strToken;
                    _this.Tranvalue(item,i,data[0].src[i].strIndex)
                }
                data.push(data[0])
                data.shift()
                // console.log(data[0].strToken)
        ...
        getview(arr,arr1){
            if(arr.strLayoutType=='1|1'){
                ...
                arr.icon='iconfont icon-sigongge02'
        ...
        Tranvalue(item,i,condition){
            console.log(item,i,condition);
            var vid='';
            if (condition) {
                setTimeout(()=>{
                    var vdata={
                ...
                if(this.rows*this.cols==4||this.rows*this.cols==9||this.rows*this.cols==16||this.rows*this.cols==25){   
                    if (i==0) {
                        vid = 'h' +11;
                    }else if (i==1) {
                        vid = 'h' +12;
        ...
        //全部暂停
        Allpause(){
            this.$store.state.stopTurePlay=false;
		...
		updateUI() {
            ...
            this.contentHeight = $('.videogh').height();
...
.liveview.tour{
    ...
    .liveview_left{
        ...
        //录像管理
        .black{
            display: none;font-size: 12px;color: #606266; padding-left: 4px;line-height: 22px;color: #f00;
        }
        .nowplay{
            display: none;font-size: 12px;color: #606266; padding-left: 4px;line-height: 26px;color: #30d158;
            .dot{
                font-size: 12px;line-height: 20px;padding-right: 4px;
            }
            .nowplayText{
                line-height: 22px;
            }
