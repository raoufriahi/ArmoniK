<template>
    <div class="liveplay h5container">
        ...
        <div :id="videonameid" class="" style="z-index:30">
        ...
        <!-- 画质 -->
        <span class="spanquality"  :id="spanqualityid">
            <el-dropdown placement='top-end' trigger="click"  @command="handleCommand" class=" "  :id="qualityid" style="z-index:30">
        ...
        <div class="btnbox" style="width: 100%;height:100%;position: absolute; top: 0;left: 0;">
       <div class="liveplay_butt">
         ...
         <div class="liveplay_ptz"  style="display:none padding:0px;z-index:100">
...
export default {
    ...
    computed: {
        listData() {
            return this.$store.state.layoutplay;
        },
        watchvideo(){
    ...
    mounted(){
        ...
        this. Gettranscod()
        if (this.$route.query.ctrl =='true') {
            $('.btnbox').css("display", "block")
        }else{
            $('.btnbox').css("display", "none")
        }
        if (this.$route.query.type) {
            var strLayoutType = this.$route.query.type;
            ...
            if (strLayoutType=="8"||strLayoutType=="9") {
                this.$nextTick(()=>{
                     $('.liveplay_butt').css('background-size', '280px 35px');
                })
            }else{
                this.$nextTick(()=>{
                    $('.liveplay_butt').css('background-size', '380px 35px');
                })
            }
        }
    },
    ...
    methods:{
        ...
        open_liveplay_butt(){
            if ($("#"+this.rtcid).attr("class")=="iconfont icon-bofang open_button rotate") {
                ...
                $("#"+this.rtcid).parent().parent('.liveplay_butt').css('background-size', '420px 35px');
            }else{
                $("#"+this.rtcid).siblings('.open_liveplay_butt').css("display", "none");
                $("#"+this.rtcid).addClass('rotate');
                $("#"+this.rtcid).parent().parent('.liveplay_butt').css('background-size', '380px 35px');
            }
        },
        //音频
        Audiosetup(){
        ...
        CloseVideo(viewparameter){
            ...
            clearInterval(this.timerRunInfo);
            //ptz
            var $controlsptz = $container.find(".liveplay_ptz");
        ...
        PtzControlShow(event){
           ...
           var url = this.$store.state.IPPORT + "/api/v1/GetPresets?token="+this.tokenshou+"&session="+ this.$store.state.token;
            ...
            var $container = $("#"+this.h5id);
            var $controls = $container.find(".liveplay_ptz");
