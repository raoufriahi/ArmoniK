<template>
  <div class="vqd">
        ...
        >
          ...
          <el-table-column label="Type">
              <template slot-scope="scope">
                <el-select v-model="scope.row.target" size="mini" placeholder="Please choose" style="width: 150px;">
                    <el-option
    ...
    >
		<div class="addtop">
			<div class="aidia">
				<div style="width:350px;padding:20px 0 20px;"><div>Numbering</div><br>
					<div>{{ai_date.token}}</div>
				</div>
				<div style="padding:20px 0 20px;"><div>Type</div><br>
					<el-select v-model="ai_date.target" size="mini" placeholder="Please choose" style="width: 100px;">
						<el-option
						v-for="item in options"
						:key="item.value"
						:label="item.label"
...
export default {
  ...
  methods: {
	  ...
	  editchaumbersure(){
			let target = {'man and car':'person_vehicle','car':'vehicle','human face':'face'};
			let time = {'3s':'3000','5s':'5000','10s':'10000'};
			this.AddChannelSloganDialog = false;
			let targets= target[this.ai_date.target]||this.ai_date.target
			let retriggertime  = time[this.ai_date.retriggertime]||this.ai_date.retriggertime
			let url = this.$store.state.IPPORT+ "/api/v2/AddTargetdetect?token="+this.ai_date.token+"&enable="+this.ai_date.enable+"&target="+targets+"&retriggertime="+retriggertime+"&session=" + this.$store.state.token;
      ...
      let url = this.$store.state.IPPORT+ "/api/v2/GetAiList?session=" + this.$store.state.token;
      this.$http.get(url).then((result) => {
		  this.tableData=[];
		  var tabledata = {}
    ...
    edit(index,date){
		...
		let targetS = {'person_vehicle':'man and car','vehicle':'car','face':'human face'};
		this.ai_date.target = targetS[date.target]
    ...
    handledel(index, row) {
      ...
      let url = this.$store.state.IPPORT+ "/api/v2/DeleteTargetdetect?token="+row.token+"&session=" + this.$store.state.token;
      this.$http.get(url).then((result) => {
        console.log(result);
        if (result.data.bStatus) {
          this.loading = false;
