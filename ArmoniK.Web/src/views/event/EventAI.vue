      ...
      <el-table class="eventTable" stripe :data="this.tableData.slice((currentPage - 1) * pageSize, currentPage * pageSize)" style="width: 100%; min-height:82vh; overflow-y: auto">
        ...
        <el-table-column prop="type" :label="G_label.Type">
          <template slot-scope="scope">
            <span v-if="scope.row.type == 'hat'">安全帽</span>
            <span v-else-if="scope.row.type == 'head'">人</span>
          </template>
        </el-table-column>
        <el-table-column prop="strTime" label="时间"></el-table-column>
        <el-table-column prop="img" width="400" label="图片">
          <template slot-scope="scope">
              <el-popover
                placement="right"
                min-width="0"
                ...
                <span slot="reference" style="cursor:pointer;"><img :src="scope.row.img" height="30"></span>
              </el-popover>
            </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    ...
    eventSearch() {
      ...
      var endtime;
      if (form.eventTime) {
        starttime = form.eventTime[0];
        endtime = form.eventTime[1];
      }
      ...
      if(this.dataToken.length>0){
         url=root + "/api/v2/GetAdvResultList?token=" +this.dataToken[0]+(starttime==undefined?'':("&starttime=" + encodeURIComponent(starttime) + "&endtime=" + encodeURIComponent(endtime))) + "&limit=" + form.limit+'&session='+this.$store.state.token;
      }else{
          if(!(form.limit=="")&& !(starttime=="")&& !(endtime=="")){
          url=root + "/api/v2/GetAdvResultList?session="+this.$store.state.token+(starttime==undefined?'':("&starttime=" + encodeURIComponent(starttime) + "&endtime=" + encodeURIComponent(endtime))) + "&limit=" + form.limit;
        }
      }
      console.log(url);
    ...
    loadEvent() {
      this.tableData = [];
      let root = this.$store.state.IPPORT;
      let url = root + "/api/v2/GetAdvResultList?limit=1000&session="+this.$store.state.token;
      this.$http.get(url).then((result) => {
        console.log(result);
        if (result.status == 200) {
          for (let i = 0; i < result.data.length; i++) {
    ...
    handleNodeClick(data, checked, indeterminate) {
      if (data.token&&data.name) {
        ...
        this.tableData = [];
        let root = this.$store.state.IPPORT;
        let url = root + "/api/v2/GetAdvResultList?token=" + data.token + "&session=" + this.$store.state.token;
        this.$http.get(url).then((result) => {
          // console.log(result);
          if (result.status == 200) {
            for (let i = 0; i < result.data.length; i++) {
